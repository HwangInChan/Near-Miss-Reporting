...
### ⚠️ 학교/회사 네트워크에서는 인식 자체가 안 될 수 있습니다

Chrome의 음성 인식은 마이크 입력을 **구글 서버로 전송해서 처리한 뒤 결과를 돌려받는**
방식으로 동작합니다. 즉, 인터넷을 통해 구글의 음성 인식 백엔드에 접근할 수 있어야 합니다.
학교·회사 네트워크 중에는 방화벽이나 프록시로 이 경로를 막아둔 경우가 있는데, 이럴 때는
마이크는 정상적으로 켜지고 "듣고 있어요" 상태까지는 되지만, 실제 인식 결과가 끝내 오지
않습니다. 코드로 해결할 수 있는 부분이 아닙니다.
...
- **음성 인식이 계속 안 된다면**: 위 "음성 인식" 섹션의 네트워크 차단 안내를 참고하세요.
